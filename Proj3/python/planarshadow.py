# planarshadow.py
import glm
from appearance import Appearance

class PlanarShadow(Appearance):
    """
    Aplica projeção planar de sombra usando luz pontual.
    """
    def __init__(self, light, plane_point, plane_normal):
        self.light = light
        self.plane_point = glm.vec3(plane_point)
        self.plane_normal = glm.normalize(glm.vec3(plane_normal))
        self._saved_matrix = None

    def _shadow_matrix(self):
        n = self.plane_normal
        a, b, c = n.x, n.y, n.z
        d = -glm.dot(n, self.plane_point)

        L = self.light.pos
        lx, ly, lz, lw = L.x, L.y, L.z, L.w

        dot = a*lx + b*ly + c*lz + d*lw

        m = glm.mat4(1.0)
        m[0][0] = dot - lx * a
        m[0][1] = -lx * b
        m[0][2] = -lx * c
        m[0][3] = -lx * d

        m[1][0] = -ly * a
        m[1][1] = dot - ly * b
        m[1][2] = -ly * c
        m[1][3] = -ly * d

        m[2][0] = -lz * a
        m[2][1] = -lz * b
        m[2][2] = dot - lz * c
        m[2][3] = -lz * d

        m[3][0] = -lw * a
        m[3][1] = -lw * b
        m[3][2] = -lw * c
        m[3][3] = dot - lw * d

        return m

    def Load(self, st):
        # Mantém a matriz original
        self._saved_matrix = st.GetCurrentMatrix()

        # Nova matriz = M_original * Sombra
        S = self._shadow_matrix()
        newMat = self._saved_matrix * S

        st.LoadMatrix(newMat)

    def Unload(self, st):
        if self._saved_matrix is not None:
            st.LoadMatrix(self._saved_matrix)
            self._saved_matrix = None
